...
class AdminService:
    ...
    def promote_user(self, target_user_id: int, admin_user_id: int) -> bool:
        """
        Назначить пользователя администратором

        Args:
            target_user_id: ID пользователя для назначения
        ...
        if not self.is_admin(admin_user_id):
            self.logger.warning(
                f"Non-admin user {admin_user_id} attempted to promote user {target_user_id}"
            )
            return False

        target_user = self.user_repo.get_user(target_user_id)
        if not target_user:
            self.logger.error(f"User {target_user_id} not found for promotion")
            return False

        if target_user.is_admin:
            self.logger.info(f"User {target_user_id} is already admin")
            return True

        target_user.promote_to_admin()
        self.user_repo.save_user(target_user)

        self.logger.info(
            f"User {target_user_id} promoted to admin by {admin_user_id}",
            extra={
    ...
    def demote_user(self, target_user_id: int, admin_user_id: int) -> bool:
        """
        Убрать права администратора

        Args:
            target_user_id: ID пользователя для разжалования
            admin_user_id: ID администратора, выполняющего действие
        ...
        if not self.is_admin(admin_user_id):
            self.logger.warning(
                f"Non-admin user {admin_user_id} attempted to demote user {target_user_id}"
            )
            return False

        # Нельзя разжаловать себя
        if target_user_id == admin_user_id:
            self.logger.warning(f"Admin {admin_user_id} attempted to demote themselves")
            return False

        target_user = self.user_repo.get_user(target_user_id)
        if not target_user:
            self.logger.error(f"User {target_user_id} not found for demotion")
            return False

        if not target_user.is_admin:
            self.logger.info(f"User {target_user_id} is not admin")
            return True

        target_user.demote_from_admin()
        self.user_repo.save_user(target_user)

        self.logger.info(
            f"User {target_user_id} demoted from admin by {admin_user_id}",
            extra={
                'admin_user_id': admin_user_id,
                'target_user_id': target_user_id,
                'target_username': target_user.username
